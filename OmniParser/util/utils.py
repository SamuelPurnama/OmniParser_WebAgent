...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    if model_name == "blip2":
        ...
        # Convert relative paths to absolute for local models
        if model_name_or_path.startswith('weights/'):
        ...
        # Use eager attention to avoid SDPA compatibility issues with Florence2
        load_kwargs = {
            "trust_remote_code": True,
            "attn_implementation": "eager"  # Required: Florence2 doesn't support SDPA
        }
        if local_files_only:
    ...
    for i in range(0, len(croped_pil_image), batch_size):
        ...
        if 'florence' in model.config.name_or_path:
            generated_ids = model.generate(
                ...
                num_beams=1,
                do_sample=False,
                use_cache=False  # Disable cache to avoid past_key_values None error
            )
        else:
